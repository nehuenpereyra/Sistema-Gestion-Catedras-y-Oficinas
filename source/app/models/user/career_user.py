...
class CareerUser(UserState):
    ...
    id = db.Column(db.Integer, db.ForeignKey('user_state.id'), primary_key=True)
    career = db.relationship("Career", back_populates="users", uselist=False)
    career_id = db.Column(db.Integer, db.ForeignKey("career.id"), nullable=False, unique=False)
